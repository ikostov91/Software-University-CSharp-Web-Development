...
namespace CurrencyConverter
{
    public partial class FormConverter : Form
    {
        ...
        private void ConvertCurrency()
        {
            ...
            {
                convertedAmmount = originalAmmount / 1.95583m;
            }
            else if (this.comboBoxCurrency.SelectedItem.ToString() == "USD")
            {
                convertedAmmount = originalAmmount / 1.80810m;
            }
            else if (this.comboBoxCurrency.SelectedItem.ToString() == "GBP")
            {
                convertedAmmount = originalAmmount / 2.54990m;
            }
            this.labelResult.Text = originalAmmount + "lv. = " + Math.Round(convertedAmmount,2) + " " + this.comboBoxCurrency.SelectedItem;
        }
    }
}
